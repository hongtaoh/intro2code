...
现在，请打开你喜欢用的文本编辑器 (text editor)，把上面的三行代码复制粘贴，然后把文件保存为 `deploy.sh`。不一定非要用 `deploy` 这个名字，你换别的也行，不过最后的那个 `.sh` 后缀一定要用。

每次我们更新完内容后，在用 `cd` 改变路径到 `wangdachui` 这个文件夹之后，只需要在终端中输入 `bash deploy.sh` 就好。
